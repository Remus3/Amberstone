...
4. **Tier the verification** (R5-R7). **Most queue rows are Tier-1:** `py_compile` plus that module's tests. Tier-0 cosmetic (doc, comment, string, non-runtime constant): Edit plus `py_compile` if `.py`. **Tier-2 is schema / engine / scorer / item-effect / `ENGINE_VERSION`** and pays the full dual suite plus the DS `:8860` restart plus the Share mirror. Say which tier you paid and why.

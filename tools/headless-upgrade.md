...
5. Model tier - haiku is the INTERIM floor for any surface still on a live call, NOT the end state (the section 4b program retires each haiku site to a precomputed path); Sonnet/Opus only where the charter requires (agent6 auditor, vision); Sonnet annotations in coaches are POST-call telemetry stamps, not call-time model picks.

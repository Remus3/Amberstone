...
- **FROZEN - do not edit (CLAUDE.md hard rule):** `tools/diagnose.md`, `tools/caveman.md`, plus anything else on the CLAUDE.md frozen list. Read-only here.
...
- **Lessons** - there is no `LESSONS.md`, and there is no live lesson-schema surface left to reconcile. The cross-Claude lesson-sync flow was decommissioned with the rest of the RC<->Peer bridge on 2026-06-24 (ADR-012): `/process-incoming-lessons` and `core/bridge_envelope.py` are both gone, and `RC_PHASE1_LESSON_SCHEMA_2026-05-02.md` now sits under `docs/_archive/2026-07-07-peer-bridge-decommissioned/` (append-only - do NOT rewrite). The only live lesson surface is the memory `feedback_*` entries, already covered by the MEMORY.md bullet above. Do not re-pitch a bridge or a lessons-sync (ROADMAP standing fence).

"""Build the objective-damage registry block for ``objdamage.py`` (item 303).

Reads the 10-channel roster-fan-out result (the workflow JSON), validates every
classified entry against ``_OBJDAMAGE_KIND_WEIGHT`` / ``_OBJDAMAGE_SCOPE_MULT``
(kind known; scope known; source in P/Q/W/E/R/BASE; magnitude in (0, 1]), then:

  * rewrites the ``# <ENTRIES>`` marker in
    ``agents/daemon_slayer/objdamage.py`` with the generated ``add(...)`` calls
...
Reusable as a drift-guard re-build when the roster scan is re-run. Usage:
  $env:LOCALAPPDATA/Programs/Python/Python314/python.exe tools/ds_objdamage_build.py <workflow_output.json>
"""
...
MODULE = ROOT / "agents" / "daemon_slayer" / "objdamage.py"
NOTES = ROOT / "agents" / "daemon_slayer" / "objdamage_registry_notes.json"
SOURCES = ("P", "Q", "W", "E", "R", "BASE")

sys.path.insert(0, str(ROOT))
from agents.daemon_slayer.objdamage import (  # noqa: E402
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: ds_objdamage_build.py <workflow_output.json>")
    ...
    for e in entries:
        ...
        if source not in SOURCES:
            ...
            continue
        if kind not in _OBJDAMAGE_KIND_WEIGHT:
            problems.append(f"unknown kind {champ} {source} {kind!r}")
            continue
        if scope not in _OBJDAMAGE_SCOPE_MULT:
    ...
    with open(NOTES, "w", encoding="utf-8", newline="") as fh:
        json.dump(
            {"item": 303, "engine": "1.115.0", "count": len(clean),
             "entries": clean},

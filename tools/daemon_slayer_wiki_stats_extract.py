...
Run from Legion (the alias + CDragon are reachable) or any host that reaches them:
  ...
  py tools/daemon_slayer_wiki_stats_extract.py --source wiki     # wiki only (legacy)
  py tools/daemon_slayer_wiki_stats_extract.py --source cdragon  # cdragon only
  py tools/daemon_slayer_wiki_stats_extract.py --source both     # default

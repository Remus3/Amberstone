"""Build the anti-tank registry block for ``antitank.py`` (item 308).

Reads the roster-fan-out result (the workflow JSON), validates every classified
entry against ``_ANTITANK_KIND_WEIGHT`` / ``_ANTITANK_CADENCE_MULT`` (kind known;
cadence known; source in P/Q/W/E/R/BASE; magnitude in (0, 1]), then:

  * rewrites the ``# <ENTRIES>`` marker in
    ``agents/daemon_slayer/antitank.py`` with the generated ``add(...)`` calls
...
Reusable as a drift-guard re-build when the roster scan is re-run. Usage:
  C:/Users/Administrator/AppData/Local/Programs/Python/Python314/python.exe tools/ds_antitank_build.py <workflow_output.json>
"""
...
MODULE = ROOT / "agents" / "daemon_slayer" / "antitank.py"
NOTES = ROOT / "agents" / "daemon_slayer" / "antitank_registry_notes.json"
SOURCES = ("P", "Q", "W", "E", "R", "BASE")

sys.path.insert(0, str(ROOT))
from agents.daemon_slayer.antitank import (  # noqa: E402
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: ds_antitank_build.py <workflow_output.json>")
    entries = _load_entries(sys.argv[1])

    problems: list[str] = []
    # Dedup on (champion, source, kind): a single ability can carry two genuine
    # anti-tank mechanisms (Trundle R = MAX_HP drain + SHRED resist-steal; Vi W /
    # Yorick E / Sion E = %HP damage + armor reduction), so the same source may
    # legitimately appear twice with different kinds. Only an exact
    # (champion, source, kind) repeat (a classify/critic overlap) is collapsed.
    ...
    for e in entries:
        ...
        if source not in SOURCES:
            ...
            continue
        if kind not in _ANTITANK_KIND_WEIGHT:
            problems.append(f"unknown kind {champ} {source} {kind!r}")
            continue
        if cadence not in _ANTITANK_CADENCE_MULT:
    ...
    with open(NOTES, "w", encoding="utf-8", newline="") as fh:
        json.dump(
            {"item": 308, "engine": "1.117.0", "count": len(clean),
             "entries": clean},

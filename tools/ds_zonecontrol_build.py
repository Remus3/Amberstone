"""Build the zone-control registry block for ``zonecontrol.py`` (item 302).

Reads the 10-channel roster-fan-out result (the workflow JSON), validates every
classified entry against ``_ZONECONTROL_KIND_WEIGHT`` /
``_ZONECONTROL_PERSISTENCE_MULT`` (kind known; persistence known; source in
P/Q/W/E/R/BASE; magnitude in (0, 1]), then:

  * rewrites the ``# <ENTRIES>`` marker in
    ``agents/daemon_slayer/zonecontrol.py`` with the generated ``add(...)`` calls
...
Reusable as a drift-guard re-build when the roster scan is re-run. Usage:
  C:/Users/Administrator/AppData/Local/Programs/Python/Python314/python.exe tools/ds_zonecontrol_build.py <workflow_output.json>
"""
...
MODULE = ROOT / "agents" / "daemon_slayer" / "zonecontrol.py"
NOTES = ROOT / "agents" / "daemon_slayer" / "zonecontrol_registry_notes.json"
SOURCES = ("P", "Q", "W", "E", "R", "BASE")

sys.path.insert(0, str(ROOT))
from agents.daemon_slayer.zonecontrol import (  # noqa: E402
...
def _emit(r: dict) -> str:
    ...
    parts = [
        ...
        f'"{r["persistence"]}", magnitude={_fmt(r["magnitude"])}'
    ]
    if r["conditional"]:
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: ds_zonecontrol_build.py <workflow_output.json>")
    ...
    for e in entries:
        ...
        if source not in SOURCES:
            ...
            continue
        if kind not in _ZONECONTROL_KIND_WEIGHT:
            problems.append(f"unknown kind {champ} {source} {kind!r}")
            continue
        if persistence not in _ZONECONTROL_PERSISTENCE_MULT:
            problems.append(f"bad persistence {champ} {source} {persistence!r}")
            continue
        ...
        clean.append({
            ...
            "persistence": persistence, "magnitude": round(magnitude, 4),
            "conditional": cond,
    ...
    with open(NOTES, "w", encoding="utf-8", newline="") as fh:
        json.dump(
            {"item": 302, "engine": "1.114.0", "count": len(clean),
             "entries": clean},
    ...
    for c in clean:
        ...
        by_pers[c["persistence"]] = by_pers.get(c["persistence"], 0) + 1
    ...
    print("persistence=" + " ".join(f"{k}:{v}" for k, v in sorted(by_pers.items())))
    if problems:

"""Build the scaling registry block for ``scaling.py`` (item 299).

Reads the 10-channel roster-fan-out result (the workflow JSON), validates every
classified entry against ``_SCALING_KIND_WEIGHT`` (kind known; source in
P/Q/W/E/R/BASE; online_stage in EARLY/MID/LATE; magnitude in (0, 1]), then:

  * rewrites the ``# <ENTRIES>`` marker in ``agents/daemon_slayer/scaling.py``
...
MODULE = ROOT / "agents" / "daemon_slayer" / "scaling.py"
NOTES = ROOT / "agents" / "daemon_slayer" / "scaling_registry_notes.json"
SOURCES = ("P", "Q", "W", "E", "R", "BASE")
STAGES = ("EARLY", "MID", "LATE")

sys.path.insert(0, str(ROOT))
from agents.daemon_slayer.scaling import _SCALING_KIND_WEIGHT  # noqa: E402
...
def _emit(r: dict) -> str:
    ...
    parts = [
        ...
        f'stage="{r["online_stage"]}", magnitude={_fmt(r["magnitude"])}'
    ]
    if r["conditional"]:
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: ds_scaling_build.py <workflow_output.json>")
    ...
    for e in entries:
        ...
        if source not in SOURCES:
            ...
            continue
        if kind not in _SCALING_KIND_WEIGHT:
            problems.append(f"unknown kind {champ} {source} {kind!r}")
            continue
        if stage not in STAGES:
            problems.append(f"bad stage {champ} {source} {stage!r}")
            continue
        ...
        clean.append({
            ...
            "online_stage": stage, "magnitude": round(magnitude, 4),
            "conditional": cond,
    ...
    with open(NOTES, "w", encoding="utf-8", newline="") as fh:
        json.dump(
            {"item": 299, "engine": "1.111.0", "count": len(clean),
             "entries": clean},

...
import lcu.snapshot_shape as _snap  # noqa: E402
from lcu.snapshot_shape import (  # noqa: E402,F401
    MASTERY_TTL_S,
    _LOBBY_QUEUE_NAMES,
    _SUMMONER_LOOKUP_TTL_S,
...
def _resolve_auth_token() -> str:
    ...
    # /agent/lcu_agent.py with no auth on a "::" bind, so the literal was
...
# -- Lobby members forwarding + mastery (moved to lcu/snapshot_shape) --------
#
# s170 (2026-05-11): the dashboard's view-lobby panel (_lobbyViewRefresh +
# _renderTop8 in web/js/main.js) expects a rich state["lobby"].members[]
# shape. That shaping, _derive_search_state, the queue-name map, and the two
# per-process caches (summoner-id lookup + mastery) were extracted VERBATIM to
# lcu/snapshot_shape.py (RC2 RM-03 E12 lever L3, 2026-07-20) so the dashboard
# can build the identical snapshot from an in-process LcuClient WITHOUT
# importing this agent. _LOBBY_QUEUE_NAMES / _SUMMONER_LOOKUP_TTL_S /
# _derive_search_state / MASTERY_TTL_S are re-exported unchanged (imported at
# the top of this file). The transport-coupled helpers below are thin wrappers
# that inject THIS module's ``lcu_request`` into the moved pure helpers, so
# every existing caller / test that patches or calls them by their agent
# names (lcu_agent._slim_lobby_member / _maybe_refresh_mastery /
# _resolve_local_summoner_id / ...) keeps working unchanged.

# Bind the two per-process caches to the shared module's objects so a test
# that mutates ``lcu_agent._mastery_cache`` in place still touches the real
# cache the moved helpers read.
_summoner_lookup_cache = _snap._summoner_lookup_cache
...
def _slim_lobby_member(m, *, local_summoner_id: int | None = None,
    ...
    """Agent-name wrapper: inject ``lcu_request`` into the moved helper."""
    return _snap._slim_lobby_member(
        lcu_request, m, local_summoner_id=local_summoner_id, enrich=enrich)


def _resolve_local_summoner_id() -> int | None:
    """Agent-name wrapper: inject ``lcu_request`` into the moved helper."""
    return _snap._resolve_local_summoner_id(lcu_request)


def _maybe_refresh_mastery() -> dict | None:
    """Agent-name wrapper: inject ``lcu_request`` into the moved helper."""
    return _snap._maybe_refresh_mastery(lcu_request)


def _reset_mastery_cache_for_tests() -> None:
    _snap._reset_mastery_cache_for_tests()
...
def capture_state() -> dict:
    ...
    # RC2 RM-03 E12 lever L3 (2026-07-20): the entire snapshot assembly (phase,
    # cs_debug, ready-check + lobby, champ-select via shape_champ_select, the
    # cherry-augment probe + game_id, mastery) was extracted VERBATIM to
    # lcu/snapshot_shape.shape_snapshot so the dashboard can build the identical
    # payload straight from an in-process LcuClient instead of round-tripping
    # this agent through the :8889 relay. The agent keeps ONLY its
    # transport-owned bits (state["config"], the lockfile-derived lcu_port) and
    # delegates the rest, so the agent-posted payload is byte-identical to what
    # it produced inline. ts is stamped inside shape_snapshot (at the END) for
    # the same reason as before: consumers see when capture finished.
    state.update(shape_snapshot(lcu_request, CONFIG))
    return state

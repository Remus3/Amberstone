...
RATING_FILE = "last_game_rating.json"; RATINGS_DIR = "data/ratings"
...
def mode_category(gm):
    ...
    return "SR"
def _ratings_dir(sd): d = Path(sd) / RATINGS_DIR; d.mkdir(parents=True, exist_ok=True); return d
...
def _identify_comp(traits,units):
    if traits:
        # Parse "Freljord 6" -> ("Freljord", 6), sort by count descending
        parsed = []
        for t in traits:
            if not t: continue
        ...
        # Pick top 2-3 traits with count > 1, or top 2 if none > 1
...
def calculate_tft_rating(placement,stage=0,level=0,game_mins=0.0,traits=None,units=None,augments=None,is_duo=False):
    ...
    # Stage/level shown in stats line — don't repeat
...
def save_tft_rating(script_dir,tft_live,tft_coaching=None):
    ...
    # Override with selected comp from comp_state + meta if live data looks stale/wrong
    # Live vision data can have set-mismatch units (OCR reading wrong set)
    try:
        import json as _j
        _sd = Path(script_dir)
        ...
        if _sel:
            ...
            if _meta_f.exists():
                ...
                if _cd:
                    # Use meta comp units as authoritative source
                    _lv8 = _cd.get("lv8") or _cd.get("lv7") or _cd.get("core_units", [])
                    _core_u = _cd.get("core_units", [])
                    # Build traits from comp name + emblem_value
    ...
    # Parse board positions from the coaching output before saving
    ...
    try:_mode_file(script_dir,"TFT").write_text(json.dumps(data,indent=2),encoding="utf-8");(Path(script_dir)/RATING_FILE).write_text(json.dumps(data,indent=2),encoding="utf-8")
    except Exception:pass
    ...
    # Rebuild placement heatmap after every saved TFT game
    try:
...
def save_rating(script_dir,champion,game_state,ally_kills_total):
    ...
    data={"rating":grade,"label":GRADE_LABEL.get(grade,""),"champion":champion,"game_mode":raw_mode,"mode_category":category,"game_time":game_state.get("game_time","0:00"),
        ...
        "notes":notes,"timestamp":datetime.now().strftime("%Y-%m-%d %H:%M")}
    try:_mode_file(script_dir,category).write_text(json.dumps(data,indent=2),encoding="utf-8");(Path(script_dir)/RATING_FILE).write_text(json.dumps(data,indent=2),encoding="utf-8")
    except Exception:pass
    db=_get_db(script_dir)
    if db:

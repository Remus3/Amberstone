...
| Daemon Slayer DPS engine | ✅ 547 items, 1346 tests, ENGINE 0.68.0; `:8893` service; all 4 coaches wired (DS-before-Haiku); dead-unique filter live; **5 of 6 archetype scorers fully wired** through `rank_for_primary_archetype()` (carry/bruiser/tank/mage/assassin); Tank EHP s174 + Bruiser hybrid s175 + CS picker s176 + Phase 4a ingest s177 + Phase 4b evaluator s178 + Phase 4c ranker s179 + **Phase 5 assassin burst-window scorer s180** (`/burst` + `/rank-assassin` + dispatcher) — only enchanter still falls back to ds.dps pending Phase 6 |

...
- **Daemon Slayer build engine** — local HTTP service on `:8893`; no runtime API cost. Computes damage-per-second, Effective HP, ability DPS, burst damage, and healing/shielding throughput for any champion × item × target combination. **DDragon purchasable coverage complete: 547 items** (SR, ARAM, Arena, Brawl) with **1979 passing tests** (ENGINE_VERSION 0.91.0, patch 16.10.1). Models armor/MR penetration, on-hit effects, spellblade procs (Triforce / Lich Bane / Essence Reaver / Iceborn / Divine Sunderer / Sundered Sky), true damage, Giant Slayer scaling, AP amps, damage amps, armor/MR shred, caster-HP-scaled amps, full Arena item mirror pool, and dead-unique candidate filtering. **Six archetype scorers** dispatch via `rank_for_primary_archetype()`: carry → `ds.dps`, tank → `ds.ehp`, bruiser → `ds.hybrid` (per-champion α/β weights from `archetype_weights.json`), mage → `ds.ability` (measured cast rates from 2851 rewind matches × 3 mode buckets), assassin → `ds.burst` (combo-window evaluator with per-champion combo templates), enchanter → `ds.hps` (curated enchanter-item formulas). Per-(champion, key) override registries cover 69% of the 171-champion roster: `champion_max_priority.json` (Q/W/E rank order), `champion_combo_sequences.json` (assassin templates), `champion_form_index.json` (form-swap routing — Nidalee cougar / Elise spider / Jayce cannon / Hwei stances / LeeSin Resonating Strike / Riven Wind Slash / Renekton Dice / AurelionSol Stardust / Qiyana Elemental), `champion_block_index.json` (canonical-amped damage block per ability key — 185 entries across 118 champions). All 4 mode coaches (SR/ARAM/Arena/Brawl) use DS-before-Haiku via `coach_integration/archetype_dispatch.py` — scorer-aware DS picks injected in the user turn before each Haiku call; legacy `daemon_slayer_picks` shape preserved for dashboard compatibility
...
**Coverage complete**: 547/547 DDragon purchasable items, ENGINE_VERSION 0.91.0, 1979 tests, patch 16.10.1. Cooldown inheritance from form 0 when non-form-0 has `cooldown=None` (Riven R / Renekton E / AurelionSol R / Qiyana Q — s206). All phases of item modeling complete. 3 items permanently deferred (Lightning Braid, Kinkou Jitte, Mejai's Arena mirror — schema limits).

**Six archetype scorers** dispatch via `rank_for_primary_archetype()` — no fallbacks remain:

| Archetype | Scorer | What it computes |
|---|---|---|
| Carry | `ds.dps` | Auto-attack DPS with full mit / on-hit / spellblade pipeline |
| Tank | `ds.ehp` | Effective HP via armor/MR factor + ARAM mod |
| Bruiser | `ds.hybrid` | α·dps + β·ehp with per-champion weights (`archetype_weights.json`) |
| Mage | `ds.ability` | Per-spell ability DPS; measured cast rates from 2851 rewind matches × 3 mode buckets (`spell_cast_rates.json`) |
| Assassin | `ds.burst` | Combo-window burst evaluator; per-champion combo templates (`champion_combo_sequences.json`); on-hit + Spellblade + Lightshield Strike per-AA proc accounting |
| Enchanter | `ds.hps` | Healing + shielding throughput; 9 hand-curated enchanter items in `data/daemon_slayer/<patch>/enchanter_items.json` |

**Per-(champion, key) override registries** (cumulative coverage 69% of 171 champions):

- `champion_max_priority.json` — Q/W/E rank order (Cassiopeia E-Q-W, Karthus Q-E-W, etc.)
- `champion_combo_sequences.json` — assassin combo templates (Zed shadow Q double, Akali R recast, Khazix isolation Q)
- `champion_form_index.json` — form-swap routing (Nidalee cougar, Elise spider, Jayce cannon, Hwei stances, LeeSin Resonating Strike, Riven Wind Slash, Renekton Dice, AurelionSol Stardust, Qiyana Elemental)
- `champion_block_index.json` — canonical-amped damage block per ability key (185 entries across 118 champions; multi-hit single-target totals, fully-charged amps, channel/duration totals, resource-state amps, execute amps)

**Coach wire-in (complete):** all 4 mode coaches (SR / ARAM / Arena / Brawl) use DS-before-Haiku via `coach_integration/archetype_dispatch.py`. Scorer-aware unit suffixes (dps/ehp/%/adps/burst/hps); legacy `daemon_slayer_picks` shape preserved for dashboard compat. Dispatcher stamps `state.cs_archetype_pick` for the active champion (decorative for the picker UI). First-purchase mismatch nudge live (`/api/archetype-nudge` + chip renderer in dashboard).
...
| Daemon Slayer DPS engine | ✅ 547 items, 1979 tests, ENGINE 0.90.0, patch 16.10.1; `:8893` service; all 4 coaches wired (DS-before-Haiku); dead-unique filter live; **all 6 archetype scorers fully wired** through `rank_for_primary_archetype()` (carry/bruiser/tank/mage/assassin/enchanter — no fallbacks); per-(champion, key) override registries cover 69% of the 171-champion roster |

...
- **Daemon Slayer build engine** — local HTTP service on `:8893`; no runtime API cost. Computes real damage-per-second AND Effective HP for any champion×item×target combination. **DDragon purchasable coverage complete: 547 items** (SR, ARAM, Arena, Brawl) with **1426 passing tests** (ENGINE_VERSION 0.69.0). Models armor/MR penetration, on-hit effects, spellblade procs, true damage, Giant Slayer scaling, AP amps, damage amps, armor/MR shred, caster-HP-scaled amps, full Arena item mirror pool, and dead-unique candidate filtering (Trinity→ER, Sterak's→Maw, Sunfire→Hollow Radiance no longer ranked). s174 adds the Tank EHP scorer (`/ehp`, `/rank-tank` routes); s175 adds the Bruiser hybrid scorer (`/hybrid`, `/rank-bruiser` routes) composing dps + ehp via per-champion (α,β) weights from `archetype_weights.json`; s176 ships the CS archetype-picker UI in the champ-select view + `GET/POST /api/cs-archetype-pick` REST endpoint + `rank_for_primary_archetype()` dispatcher; s177 ships Phase 4a champion ability ingest — `agents/daemon_slayer/abilities.py` + `tools/daemon_slayer_abilities_extract.py` consuming Meraki bulk; 171/172 champions / 927 ability forms with 98.4% ok / 99.3% parsed; s178 ships Phase 4b mage ability DPS evaluator — `agents/daemon_slayer/ability_dps.py` + `scripts/build_spell_cast_rates.py` (Q/W/E/R measured casts/sec from 2851 rewind matches × 3 mode buckets) + `/ability-dps` route; s179 ships Phase 4c mage ability DPS ranker — `rank_items_by_ability_dps()` + `AbilityDpsRankedItem` + `/rank-mage` route + `rank_mage_for()` / `ability_dps_for()` client helpers, plus the dispatcher's mage branch now routes to `ds.ability`; s180 ships Phase 5 assassin burst-window scorer — `agents/daemon_slayer/burst.py` (`compute_burst_damage()` + `rank_items_by_burst()` + `BurstResult` + `BurstRankedItem` + `ComboCast`) + `/burst` + `/rank-assassin` routes + `rank_assassin_for()` / `burst_for()` client helpers, plus the dispatcher's assassin branch now routes to `ds.burst`; s181 ships Phase 6 enchanter healing throughput scorer — `agents/daemon_slayer/hps.py` (`compute_hps()` + `rank_items_by_hps()` + `HpsResult` + `HpsRankedItem` + `EnchanterFormulasSnapshot`) + new `data/daemon_slayer/16.9.1/enchanter_items.json` (9 curated enchanter items: Moonstone, Redemption, Mikael, Helia, Ardent, Staff of Flowing Water, Locket, Mandate, Knight's Vow) + `/hps` + `/rank-enchanter` routes + `rank_enchanter_for()` / `hps_for()` client helpers, plus the dispatcher's enchanter branch now routes to `ds.hps` (**all 6 archetype scorers fully wired through `rank_for_primary_archetype()`** — carry/bruiser/tank/mage/assassin/enchanter; no fallbacks remain; archetype-expansion plan complete); **s182 ships the coach archetype-dispatch wire-in** — new `coach_integration/archetype_dispatch.py` resolves the operator's primary archetype for the active champion + calls the dispatcher + returns `CoachDispatchResult` with scorer-aware `picks_str` (unit suffix per archetype: dps/ehp/%/adps/burst/hps) + `display_rows` (legacy `daemon_slayer_picks` shape preserved); all 4 mode coaches (SR/ARAM/Arena/Brawl) swap their `rank_for()` calls to the helper so DS picks match the champion's archetype; `dashboard/routes_state._serve_ds_preview_post` routes through the dispatcher with optional `archetype` payload override; `dashboard/_state_builder.build_state()` stamps `state.cs_archetype_pick` for the active champion (decorative for the picker UI; coaches resolve independently). All 4 coaches (ARAM, Arena, Brawl, SR) use DS-before-Haiku — picks injected in user turn before LLM call
...
**Coverage complete**: 547/547 DDragon purchasable items, ENGINE_VERSION 0.69.0, 1426 tests. All phases of item modeling complete. 3 items permanently deferred (Lightning Braid, Kinkou Jitte, Mejai's Arena mirror — schema limits). s174 adds the Tank EHP scorer (`ehp.py`); s175 adds the Bruiser hybrid scorer (`hybrid.py`); s176 adds the CS archetype-picker UI + `rank_for_primary_archetype()` dispatcher (`core/archetype_picks.py` + `dashboard/routes_archetype.py` + 6-button picker in champ_select.js My Pick card); s177 adds Phase 4a champion ability ingest (`agents/daemon_slayer/abilities.py` + `tools/daemon_slayer_abilities_extract.py`) — 171/172 champions / 927 ability forms / 98.4% ok parse; s178 adds Phase 4b mage ability DPS evaluator (`ability_dps.py`) with measured cast rates from `spell_cast_rates.json` (172 champs × Q/W/E/R × SR/ARAM/ARENA, derived from 2851 rewind matches by `scripts/build_spell_cast_rates.py`); s179 closes the Phase 4 lift with `rank_items_by_ability_dps()` + `AbilityDpsRankedItem` / `AbilityDpsRankResult` + `/rank-mage` route + `rank_mage_for()` / `ability_dps_for()` client helpers + dispatcher integration (mage → `ds.ability` instead of falling back to ds.dps); s180 ships Phase 5 assassin burst-window scorer (`burst.py`) with `compute_burst_damage()` + `rank_items_by_burst()` + `ComboCast` + `BurstRankedItem` + `/burst` + `/rank-assassin` routes + `rank_assassin_for()` / `burst_for()` client helpers + dispatcher's assassin branch routing to `ds.burst`; **s181 ships Phase 6 enchanter healing throughput scorer** (`hps.py`) with `compute_hps()` + `rank_items_by_hps()` + `HpsResult` + `HpsRankedItem` + `EnchanterFormulasSnapshot` + new hand-curated `data/daemon_slayer/16.9.1/enchanter_items.json` (9 enchanter items: Moonstone / Redemption / Mikael / Helia / Ardent / Staff of Flowing Water / Locket / Mandate / Knight's Vow) + `/hps` + `/rank-enchanter` routes + `rank_enchanter_for()` / `hps_for()` client helpers + dispatcher's enchanter branch routing to `ds.hps`. **All 6 archetype scorers fully wired** through the dispatcher — carry/bruiser/tank/mage/assassin/enchanter — no fallbacks remain; archetype-expansion plan complete. Coach integration deferred — picker persists state but no coach reads `state.cs_archetype_pick.primary` yet.
...
| Daemon Slayer DPS engine | ✅ 547 items, 1426 tests, ENGINE 0.69.0; `:8893` service; all 4 coaches wired (DS-before-Haiku); dead-unique filter live; **all 6 archetype scorers fully wired** through `rank_for_primary_archetype()` (carry/bruiser/tank/mage/assassin/enchanter — no fallbacks); Tank EHP s174 + Bruiser hybrid s175 + CS picker s176 + Phase 4a ingest s177 + Phase 4b evaluator s178 + Phase 4c ranker s179 + Phase 5 assassin burst-window scorer s180 + **Phase 6 enchanter HPS scorer s181** (`/hps` + `/rank-enchanter` + dispatcher; 9 curated enchanter items) — archetype-expansion plan complete |

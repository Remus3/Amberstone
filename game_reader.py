# arch: reads cached liveclient + frames; produces coaching snapshots (god module) | section=vision | frozen=no
"""
game_reader.py — Reads League of Legends game state from Riot's local APIs.
...
LCU API (client/champ select): https://192.168.8.237:{port}/
  - Auth from LeagueClientUx process command line
...
class GameReader:
    ...
    def _process_game(self, raw):
        ...
        # TFT has different data structure; the dedicated TFT coach handles
        # full state parsing via tft_state_reader.py
        if "TFT" in game_mode:
            # Check for GameEnd before returning minimal state
            for ev in events:
        ...
        _max_t = 3 if game_mode in ("ARAM", "KIWI", "ARAM_5V5", "ARAM_MAYHEM") else 11

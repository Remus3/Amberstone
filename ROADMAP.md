...
- 🟡 **Phase 3 step 3 follow-ups — central panel + enemies redesign for ALL game modes** — operator parking-lot for s165. Current scaffold is SR-draft-shaped; ARAM (no bans, bench swaps, no role assignment), Arena (2v2v2v2, augments), Brawl (random 5v5) need mode-conditional layouts on the central My Pick + Build Chooser pane AND the enemies panel. Pick&Ban + Allies panels are LOCKED — don't re-iterate.
- 🟡 **Phase 3 fixture flow (steps 4–14)** — remaining canonical fixtures: champ-select 1st/5th/locked variants, loading screen, active match @ 0/6/12/20/30/40 min, game summary (NEW view), last match. Per `feedback_phase3_fixture_ritual.md` — each fixture begins with visual-hierarchy audit subagent, ends with `/done` + `/clear`.
- 🟡 **LCU agent handlers for champ-select trade/swap intents** — Phase B work in `tools/gamepc_lcu_agent.py`. Dashboard fires `set_ban_intent` / `set_pick_intent` / `request_position_swap` / `request_pick_order_swap` and expects `cs.active_round` populated from LCU `actions[]` array. All currently no-op on the agent side; commands queue but don't execute. Also verify `trade_request` works for SR champion trades (already wired for ARAM bench swaps).

...
- 🟡 **Phase 3 fixture flow (steps 2–14)** — step 2 (`flow_02_lobby_with_others`) is next, mostly a renaming pass since flow_01 already exercises 5-member layout via placeholder-driven slots. Remaining fixtures: champ-select 1st/5th/locked, loading screen, active match @ 0/6/12/20/30/40 min, game summary (NEW view), last match. Per `feedback_phase3_fixture_ritual.md`: each fixture begins with visual-hierarchy audit subagent, ends with `/done` + `/clear`. Phase B LCU wiring queue captured in WAKEUP_NOTES s163.

...
can exist and it covers every caller of `connect()` - today the frozen
auto-accept tick and the rune writer's poll, both at 1 Hz on the one shared
client, whose duplicate line was 38-46 percent of every emission measured on
the daily logs. See `lcu/lockfile_notice.py` for the measurements and for why
the unit of suppression is the gap EPISODE, not the throttle window.
"""

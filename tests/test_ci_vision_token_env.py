...
MEASURED 2026-09-06. `dcd965f2d` untracked `config/vision_token.txt`, which had
...
2. The value is NOT shaped like the real token. `_resolve` does no format
   ...
   `dcd965f2d` removed, and it would land wearing the costume of a bug fix.
...
def test_ci_supplies_a_vision_token_to_every_job():
    ...
    assert found, (
        ...
        "dcd965f2d - 120 failed, 53 errors, all one cause.")
...
def test_the_ci_vision_token_is_not_a_real_one():
    """A real token in a tracked file is the state dcd965f2d removed."""
    for where, value in _declared_vision_tokens().items():
        ...
        assert not _REAL_TOKEN_SHAPE.match(value.strip()), (
            ...
            f"of a REAL vision token. dcd965f2d untracked exactly such a value; "

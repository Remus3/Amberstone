...
engine. The fix is a deterministic, engine-less regen (no :8893 required):
    ...
    py -m core.build_order_precompute --static --mode all
    py -m core.build_order_variants   --static --mode all
...
def test_build_order_precompute_stamp_tracks_engine(mode: str) -> None:
    ...
    assert payload, (
        ...
        f"regen: py -m core.build_order_precompute --static --mode all"
    )
...
def test_build_order_variants_stamp_tracks_engine(mode: str) -> None:
    ...
    assert payload, (
        ...
        f"{patch}; regen: py -m core.build_order_variants --static --mode all"
    )
    assert payload.get("engine_version") == ENGINE_VERSION, (
        f"HZ-B2 build_order_variants_{mode}.json stamped "
        f"{payload.get('engine_version')!r}, engine is {ENGINE_VERSION!r}; "
        f"regen: py -m core.build_order_variants --static --mode all"
    )

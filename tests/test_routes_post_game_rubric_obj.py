...
def _seed_obj_db(
    ...
    """Widened seed: carries team_id + the 6 objective columns."""
    ...
    conn.execute(
        ...
        (match_id, 100, operator_puuid, team_position) + _obj_tuple(operator_objectives),
    )
    ...
    conn.execute(
        ...
        (match_id, 100, "ALLY_A", "TOP") + _obj_tuple(ally_objectives),
    )
    conn.commit()
    conn.close()

...
This guard pins the SIGNAL without touching the contract:
  ...
  * the throttle holds - ``build_state()`` runs on the SSE/TTL cadence
    (``dashboard/routes_state.py:210-230`` + :378-390), so a persistent fault
    must not flood the log. Cadence is ``RC_STATE_CADENCE_SEC``, default 0.5 s
    with a 0.1 floor (``dashboard/routes_state.py:167-178``), i.e. about 2 Hz -

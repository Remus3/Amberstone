...
## s74 hand-off — 2026-05-04 (mode-aware resolver + ARAM target_bonus_hp activation)

Single-arc continuation. Took s73 #9 candidate (ARAM coach
target_bonus_hp activation, "easiest port from arena_coach since
alias-ID path is identical"). Turned out the prerequisite — making
the resolver mode-aware so non-Arena callers get base IDs not 22XXXX
aliases — was substantial enough to be the lead change. ARAM
activation rode on top once the resolver was right.

Skipped s73 #1 (wiki scraper, 3-5hr too big for one session) and
#2 (SR coach, needs vision-side enemy item parsing first; ARAM gets
items free from LCU `allPlayers[*].items[]`, SR doesn't surface
enemy items the same way).

**Pattern decision worth pinning — the legacy byName quirk
(``items_index.json`` first-seen-wins picking 22XXXX aliases) is
load-bearing for arena_coach but a silent foot-gun for everyone
else.** Don't try to fix the quirk at the pipeline layer — that
would break arena_coach's correctness which depends on the alias
path. Instead, expose a second resolution mode that reads DDragon's
``maps`` field directly and fans out into per-mode byName indices.
arena_coach pins ``mode='arena'`` to make its correctness intentional;
ARAM/SR/Brawl coaches pin their respective modes to dodge the alias
trap. Backward-compat default (``mode=None``) preserves the quirk for
any caller that doesn't yet care about HP/cost mode-correctness.

**Pattern decision worth pinning — coach-side resolver caches
should refresh on the same trigger when their data sources are
the same file.** ``_load_hp_if_stale`` now populates both
``_hp_cache`` and ``_byname_by_mode`` in a single pass over
``data/daemon_slayer/<patch>/items.json``. Splitting them into two
loaders would have meant two reads of the same file on cold-cache;
the lazy-load pattern stays cheap because it's mtime-gated and we're
only paying the parse cost once per patch refresh.

**Pattern decision worth pinning — purchasable=False filter on
mode index.** DDragon items.json contains recipe shells / quest
items that share names with real legendaries (rare but real).
Skipping ``purchasable: false`` entries during the per-mode index
build prevents a quest shell from shadowing its legendary
counterpart. Doesn't affect HP cache (which keys by ID, not by
name). Doesn't affect items_index.json byName legacy path (that's
externally generated by the data pipeline).

**Shipped (commit `e300de2`, will push):**

- `core/daemon_slayer_resolver.py`:
  - New ``_MODE_TO_DDRAGON_MAP_ID`` constant: sr=11, aram=12, arena=30,
    brawl=35 (DDragon's ``maps`` field is keyed by these integers as
    strings).
  - New ``_byname_by_mode: dict[str, dict[str, str]]`` cache.
  - ``_load_hp_if_stale`` extended to build ``_byname_by_mode`` in
    the same pass. Skips ``purchasable: false`` entries; first-write-
    wins per mode otherwise (debug log on collision, never raises).
  - ``name_to_id(name, mode=None)`` accepts optional mode; routes
    through per-mode index when supplied. Unknown mode falls through
    to legacy lookup (defensive against typos in coach config).
  - ``resolve_many(names, mode=None)`` forwards mode kwarg.
  - Module docstring documents the mode table + the alias-ID
    backstory.

- `coaches/arena_coach.py`:
  - ``_estimate_target_bonus_hp`` resolves with ``mode='arena'``
    (was relying on byName quirk implicitly). Same behavior, now
    intentional.
  - ``_run_coach`` /rank wire-in resolves ``owned_ids`` with
    ``mode='arena'``. Same reason.
  - ``s73→s74`` comment markers updated.

- `coaches/aram_coach.py`:
  - ``_to_state`` populates new ``enemies: list[{name,is_dead,items}]``
    field. Sibling of existing ``enemy_items`` (string for Haiku
    prompt) and ``enemy_comp`` (flat champ name list); no shift of
    pre-existing fields, so dashboard renderers + ARAM Haiku prompt
    are unchanged.
  - New ``_estimate_target_bonus_hp(state)`` method. Walks
    ``state['enemies']``, filters ``not is_dead``, resolves with
    ``mode='aram'``, sums HP per opponent, returns MAX clamped at
    1500. **No round-count fallback** (ARAM has no rounds): vision
    gap returns 0.0 = "no signal" and the engine no-ops Giant
    Slayer's amp.
  - ``_run_coach`` gains a daemon_slayer wire-in block. Mirrors
    arena_coach's Phase 7 pattern: lazy-imports client + resolver,
    estimates target_bonus_hp, calls ``rank_for(mode='ARAM',
    target_bonus_hp=...)``, surfaces ``daemon_slayer_picks`` on
    response. Engine down → field absent (no regression to existing
    Haiku output).

- `tests/phase2_smoke/test_daemon_slayer_resolver_modes.py` (NEW, 19 tests):
  - Per-mode resolution pins (Heartsteel/Riftmaker SR vs Arena vs ARAM).
  - Case-insensitive mode normalization.
  - mode=None legacy quirk preservation.
  - Unknown-mode fallthrough (e.g. 'tft').
  - Empty/None name → None.
  - resolve_many round-trip preserving order, skipping unknowns.
  - End-to-end HP flow-through (resolve → bonus_hp_for_id) for ARAM
    (900) vs Arena (700) Heartsteel.

- `tests/phase2_smoke/test_aram_coach_target_bonus_hp.py` (NEW, 11 tests):
  - No state / empty enemies / all dead / no items → 0.0.
  - Single alive tank w/ Heartsteel+Riftmaker → 1250 (ARAM HP values
    sum, NOT Arena alias values).
  - 5-enemy team with one tank → 1250 (MAX, NOT avg-of-team 250).
  - Dead enemy with Heartsteel excluded; live enemy with only
    Riftmaker → 350.
  - Pen-only build (LDR + Mortal Reminder) → 0.
  - 4-stack tank build clamps at 1500.
  - **Regression pin** (`test_uses_aram_mode_not_arena_aliases`):
    Heartsteel alone → 900 (NOT 700). If this test ever returns 700,
    the resolver mode argument got dropped somewhere.

**Test state:**
- `agents/daemon_slayer/tests/`: 379/379 (unchanged from s73).
- `tests/phase2_smoke/`: **136/136** green (s73 baseline 106 + 30
  new from this batch — 19 resolver-modes + 11 aram-coach).
- Pre-existing stale `0.9.3` engine_version pin in
  `test_sr_draft_profile_engine` still failing (carried from s73,
  out of scope).

**Live verify (post `restart_trigger.txt`, pid 7316 → 4864):**

Resolver smoke check:
```
Heartsteel mode=aram   -> 3084   HP=900.0   ✓
Heartsteel mode=arena  -> 223084 HP=700.0   ✓
Heartsteel mode=sr     -> 3084   HP=900.0   ✓
Heartsteel mode=brawl  -> 3084   HP=900.0   ✓
Heartsteel mode=None   -> 223084 (legacy)   ✓
Heartsteel mode='tft'  -> 223084 (fallthru) ✓
```

Engine /rank ARAM smoke (Sett L11 owns Heartsteel, target_bonus_hp=1250):
```
Trinity Force      d_dps=47.94 gold=3333
Rapid Firecannon   d_dps=45.91 gold=2650
Statikk Shiv       d_dps=44.94 gold=3000
```
Engine accepts ``mode="ARAM"`` + ``target_bonus_hp`` happily.
ARAM coach wire-in dormant (no ARAM game in progress).

**Decisions worth pinning:**
- **Mode-aware resolver was the actual unblocker** for everyone past
  arena_coach. ARAM was easy because LCU surfaces enemy items
  directly; SR will be harder (needs vision-side enemy item parsing).
- **Per-mode byName built from items.json `maps`, not from
  items_index.json** — the resolver now has two parallel byName
  paths (legacy quirk + per-mode). Don't merge them; they serve
  different masters. arena_coach explicitly relies on the quirk
  path NOT being mode-aware (mode=None default).
- **Structured `enemies` field on ARAM state, not on
  `enemy_items`** — the latter is a "; "-joined string consumed by
  Haiku prompt formatting; touching it would cascade through
  prompt template + log parsing. New struct is additive.

**Things tomorrow-you should NOT redo:**
- Don't switch ARAM estimator to use ``enemy_items`` string. Re-
  parsing a "; "-delimited blob just to extract per-enemy items
  is fragile; the new ``enemies`` struct is the canonical shape.
- Don't add a round-count or game-time fallback to ARAM
  ``_estimate_target_bonus_hp``. Arena has rounds, ARAM doesn't.
  0.0 is the correct "no signal" emit for ARAM vision gaps.
- Don't change ``mode=None`` default behavior. arena_coach
  re-pinning relies on the byName quirk staying unchanged for
  callers that don't pass mode explicitly. Future SR coach
  activation should pin ``mode='sr'`` even though ``mode=None``
  happens to return Arena alias IDs (which would be subtly
  wrong for SR HP).
- Don't merge ``_byname_by_mode`` populating into the
  ``_load_if_stale`` (items_index) path. They use different
  source files (DDragon items.json vs pipeline-built
  items_index.json) with different lifecycles.

**Activation:** RC main restarted via ``restart_trigger.txt`` (pid
7316 → 4864, last_reload_ok=true). aram_coach now resolves enemy
items in ARAM mode (gets SR base HP values not Arena alias HP),
estimates target_bonus_hp from MAX over alive opponents, and surfaces
``daemon_slayer_picks`` on the engine response. Behavior dormant
until next ARAM game starts.

**Bridge state at session end:** RC main pid=4864 alive=true
reload_ok=true. Engine on :8893 = 0.23.0 unchanged. LCU
phase=None (no game in progress). Bridge to Game-PC last result
~1493s ago at session start; no two-way traffic this session.

**Operational backlog (delta from s73):**
- ✅ ~~ARAM coach target_bonus_hp activation~~ (was s73 #9).
- ✅ ~~Mode-aware HP lookup~~ (was s73 NEW backlog item, became
  the lead change of s74).
- All other s73 backlog unchanged: Hullbreaker Skipper blocked,
  SR coach target_bonus_hp blocked on vision-side enemy item
  parsing, stale 0.9.3 engine_version pin, etc.

**Next-session candidates (ranked):**
1. **Phase 4 batch 20: League wiki scraper** (carried s71 #2 →
   s72 #2 → s73 #1 → s74 #1). Unblocks Hullbreaker / Essence
   Reaver / Sterak's coefficient gaps. ~3-5 hour scope.
2. **SR coach `target_bonus_hp` activation** (carried). Probably
   needs vision-side enemy item parsing first. ARAM was free
   because LCU surfaces all items via ``allPlayers[*].items[]``;
   SR doesn't expose enemy items the same way (fog of war + LCU
   filters).
...
7. **Activate arena augment v2 in production** (carried).
8. **Per-target-HP-pct field** (carried; defer until caller demands).
9. **Brawl coach `target_bonus_hp` activation** (NEW from s74).
   Brawl is map 35; resolver mode='brawl' is ready. Whether

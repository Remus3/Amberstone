...
23. 🟡 **LCU agent handlers for champ-select trade/swap + Arena augment intents** — Phase B work in `tools/gamepc_lcu_agent.py`. Dashboard fires `set_ban_intent` / `set_pick_intent` / `request_position_swap` / `request_pick_order_swap` and (new in s165) `set_augment_intent`; expects `cs.active_round` populated from LCU `actions[]` array + (new) `cs.arena_teams` / `cs.augments.{my_slots, options, current_round}` / `cs.is_brawl` populated for non-SR modes. All currently no-op on the agent side; commands queue but don't execute. Also verify `trade_request` (already supported for ARAM bench swaps) works for SR champion trades. Also: real build-chooser variants currently static placeholders — wire to `/api/loadout/list` keyed on champion + mode.

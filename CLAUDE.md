...
20. 🟡 **Phase 3 fixture flow (steps 2–14)** — step 2 `flow_02_lobby_with_others` is next session's start point. Per `feedback_phase3_fixture_ritual.md`: visual-hierarchy audit subagent first on `flow_01_lobby_solo`, then move to step 2. Phase B LCU wiring queue (mastery / position-prefs / party-type / auto-accept / invite/kick/promote / top role champs / friends online status) captured in WAKEUP_NOTES s162.

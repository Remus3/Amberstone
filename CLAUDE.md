...
21. 🟡 **Phase 3 fixture flow (steps 2–14)** — step 2 `flow_02_lobby_with_others` is next session's start point (renaming pass since flow_01 already exercises 5-member layout). Then step 3 Champ-Select. Phase B LCU wiring queue captured in WAKEUP_NOTES s163: assessed_role + averaged.kp + averaged.avg5 + position_preferences read-direction + per-mode game breakdown (rift/aram/arena).

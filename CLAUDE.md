...
26. 🟡 **Deferred post-s167:** (a) Game-PC redeploy of `tools/gamepc_lcu_agent.py` so mastery field appears live (Legion update only, peer copy stale); (b) **P6** Sonnet/Haiku → Claude Desktop API key — needs operator clarification on which key file before code change (today's path: `coaches/_base_coach.py:read_api_key()` reads `API-Key-Claude.txt` then `$ANTHROPIC_API_KEY` env); (c) **P1b** Arena augment registry expansion — needs `as_pct` overlay channel (architectural, not 1-hr); (d) **P5** Default DS build #4 — waits on richer rewind data (currently sparse: 5 games since Dec 2025); (e) **P3/P4** History season-WR + Replay tab — UI work, deferred per operator.

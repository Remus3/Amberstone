...
- **Git hooks are the AUTHORITATIVE gate, and a fresh clone has NONE.** `core.hooksPath` is LOCAL config and is not cloned, so a new clone of this repo runs zero hooks until someone sets it - which defeats the tracked `.githooks/` dir entirely. **First action in any fresh clone: `python scripts/install_hooks.py`.** Claude PreToolUse hooks are the fast in-session signal only - MEASURED 2026-07-26, they do NOT survive a headless `claude -p --permission-mode bypassPermissions`, which committed a banned glyph straight through while the git hooks blocked it. Never treat a hook's PRESENCE as proof it fires; the only valid test is end-to-end (stage a banned glyph, attempt a real commit, assert HEAD unchanged). See memory `reference_git_hooks_authoritative_and_traps` for the four ways a hook can be present and silently do nothing.

...
|---|---|---|---|
| **Legion** | `legion-rc` / `100.70.22.55` | `192.168.8.230` | 1-PC (2026-05-29, ADR-011): runs League + Vanguard + RC + supervisor + vision server + dashboard + OBS. Relocated agents run local as ONLOGON tasks: RC-LCUAgent / RC-LiveClientRelay / RC-HotkeyListener. Tailscale node stays `legion-rc`. Windows computer name is `DESKTOP-TNNVA65` (re-measured 2026-08-02: `$env:COMPUTERNAME` + both registry `ComputerName` keys + CIM agree; the earlier `DESKTOP-JKZECV9` was stale). `hostname` returns `LEGION-RC` because the DNS name is set separately - the two disagreeing is expected, so never "reconcile" them |

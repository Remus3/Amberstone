...
18. 🟡 **Active Match view** — steps 1–3 shipped; steps 4–5 pending. Step 1 scaffold (s159 `3f72795` / s160 `57886e1` / s161 `8a857c4`). Step 2 + 3 shipped s170 (`85461e2`): DS icons + OWNED overlay + +Ndps delta caption in `web/js/panels/active_match.js`; per-tick rerank via `/api/ds-preview` (4s cooldown + input fingerprint); enemy-stats helper `coach_integration/enemy_stats.py` replaces hardcoded `target_armor=80.0` across all 4 coaches with mode + level scaled curve (27 tests). **Steps 4–5 pending:** (4) static SR/ARAM/Arena/Brawl map + ZOI/threat/gank/MIA overlay, (5) zen-lock in-game + RIGHT NOW fold + bridge-pending → dev-panel button + fleet view removal.

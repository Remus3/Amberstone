...
**Cross-repo channel with Sibling-A (`C:\Sibling-A` - a REAL SPACE since 2026-09-06, LW commit 81de837; the GitHub repo keeps the hyphen as `the Sibling-A repo` because a repo name cannot hold a space, so the two spellings differ ON PURPOSE and neither is a typo - a spelling anchored on `C:` is a filesystem path and takes the space, a bare token is the repo or project name. Quote it: an unquoted `-File C:\Sibling-A\tools\x.ps1` reads as `-File C:\Legion` plus a stray positional and fails silently):** gitignored `moon_sync_inbox/` in EACH repo root - you WRITE into the sibling's, you READ your own. Both sessions independently invented a different channel on 2026-07-26 before finding this one; do not invent a third. `ops/loop/slots.py` + `ops/loop/winmutex.py` are BYTE-IDENTICAL-BY-CONTRACT across the participating repos, pinned by `SHARED_SHA256` in `tests/test_loop_concurrency.py`. Re-pinning is a JOINT act: never regenerate the digests from local disk - both trees hashing equal IS the acceptance, not a note claiming it. Copy the sibling's file with a BYTE-level copy, never `write_text` (it turns LF into CRLF on Windows and the pin is on bytes). **Participants since 2026-09-06: RC, Sibling-A and Sibling-B (`C:\Sibling-B`, with a SPACE). Sibling-C is archived and its working copy deleted; it will never re-sync, so do not chase its digest.** RSC vendors last - it has no pin to break until it has one.
...
- **R1** Files = Read / Edit / Write / Grep / Glob ONLY. NEVER computer-use / Windows-MCP to read or change a file. **This bans Bash-mediated WRITES to repo files too** - `python - <<'EOF'` heredocs, `sed -i`, `>` redirection - and not only for tidiness: `tools/stop_claim_gate.py` corroborates every file claim from the Edit/Write tool-call record, so a file changed through Bash reads to it as an UNBACKED CLAIM even when the change is real and committed. Measured 2026-09-04: CLAUDE.md was corrected via a heredoc, committed as `b28616286`, and the gate blocked the wrap twice over a true statement. Bash stays correct for READING (`grep`, `git show`) and for scratchpad files; for a tracked file, use Edit/Write. If you already used Bash, back the claim with `git show <sha> -- <path>` rather than retracting something true - and never fabricate an Edit call to satisfy the gate.

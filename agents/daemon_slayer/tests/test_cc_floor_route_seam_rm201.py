...
class StrandedLedgerTests(unittest.TestCase):
    ...
    def test_the_sibling_row_landed_by_being_wired_not_by_being_dropped(self) -> None:
        """RM-200 owns assume_scaling_hsp_grants and shipped in the same batch.

        This assertion was authored as ``assertIn(..., STRANDED_DEPTH1)`` while
        this slice was alone on its branch, where it correctly pinned "I did not
        touch the sibling row". Both slices then landed together and it went RED
        at the merge - the guard working, not a break, and the reason a slice
        must never pin a SIBLING slice's in-flight state.

        Rewritten to pin the property that actually matters and that a deletion
        cannot fake: the sibling left the ledger by being WIRED. A bare
        ``assertNotIn`` would pass just as well if the seam had been deleted from
        the engine outright, which is the failure this batch exists to prevent.
        """
        sibling = "assume_scaling_hsp_grants"
        self.assertNotIn(sibling, STRANDED_DEPTH1)
        self.assertIn(sibling, _parsed_keys(_SERVER_SRC))
        self.assertIn(sibling, _passed_kwargs(_SERVER_SRC))

    def test_the_depth1_ledger_is_empty_and_that_is_a_floor_not_a_silence(self) -> None:
        """Both names left; the guard must still be able to REPORT a new one."""
        self.assertEqual(STRANDED_DEPTH1, {})
        self.assertEqual(stranded_seams_depth1(_SERVER_SRC), {})
        # Non-vacuity: strip this seam's forwarding and the checker must speak
        # again. An empty ledger that cannot go non-empty is not a floor, it is
        # a dead guard.
        stripped = _SERVER_SRC.replace(f"{_SEAM}={_SEAM},", "")
        self.assertNotIn(_SEAM, _passed_kwargs(stripped))

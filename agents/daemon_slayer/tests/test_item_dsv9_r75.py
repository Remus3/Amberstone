...
class EngineVersion(unittest.TestCase):
    ...
    def test_engine_version_bumped(self) -> None:
        self.assertEqual(ENGINE_VERSION, "1.280.0")
        self.assertEqual(daemon_slayer.ENGINE_VERSION, "1.280.0")
...
class ComputeDpsSurface(unittest.TestCase):
    ...
    def test_kwarg_end_appended_on_both_consumers(self) -> None:
        ...
        # advancing this end marker from assume_shielded_target. RM-172
        # (2026-08-06) then END-appended apply_mode_modifiers to BOTH consumers
        # while wiring that seam uniformly across the DS route seam, advancing
        # both markers again. The convention this test guards is unchanged and
        # is asserted below: a new kwarg goes at the END, so the previous end
        # markers must still sit after every pre-R110 seam.
        self.assertEqual(burst_params[-1], "apply_mode_modifiers")
        self.assertEqual(burst_params[-2], "assume_item_lowhp_magic_crit")
        ...
        self.assertEqual(dps_params[-1], "apply_mode_modifiers")
        self.assertEqual(dps_params[-2], "apply_passive_aura_damage")
        self.assertEqual(dps_params[-3], "assume_item_lowhp_magic_crit")
        self.assertLess(
            dps_params.index("assume_shielded_target"),
            dps_params.index("assume_item_lowhp_magic_crit"),
        )
        # The burst consumer keeps the same ordering invariant.
        self.assertLess(
            burst_params.index("assume_shielded_target"),
            burst_params.index("assume_item_lowhp_magic_crit"),
        )

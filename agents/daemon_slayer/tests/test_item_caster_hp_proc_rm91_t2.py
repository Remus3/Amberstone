...
class SignatureConventionTests(unittest.TestCase):
    ...
    def test_kwargs_are_appended_last_with_defaults_off(self) -> None:
        ...
        names = list(params)
        # RM-118 (2026-07-29) appended ``assume_hsp_amp`` after this pair, and the
        # RM-118 MANA lane (2026-08-02) appended its own coupling pair after that,
        # so the T2 pair keeps sliding further from the literal tail. The pair must
        # stay CONTIGUOUS and correctly defaulted - that is the actual convention
        # this guard protects, not a claim to be the literal tail. A new seam
        # appends AFTER these and re-offsets this guard.
        self.assertEqual(
            tuple(names[-2:]),
            ("apply_mana_damage_coupling", "mana_coupling_strength"),
        )
        self.assertEqual(names[-3], "assume_hsp_amp")
        self.assertEqual(
            tuple(names[-5:-3]),

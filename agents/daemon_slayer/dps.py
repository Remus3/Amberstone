"""Phase 2 step 2 — auto-attack DPS over lolmath rotation scenarios.
...
Ability damage is **not** included — those formulas aren't in the snapshot
yet (Phase 4 territory). Only the basic-attack portion of each rotation
is scored; rotation duration includes the time spent casting abilities,
so longer rotations naturally dilute auto-attack DPS.
"""
...
# Base bonus crit damage on auto-attacks (no Infinity Edge). IE adds ~+40%
# to this in current League; that's a Phase 4 conditional.
...
def _rotation_attack_dps(
    ...
    target_armor: float,
    mode_dmg_mult: float,
    crit_bonus: float = DEFAULT_CRIT_BONUS,
) -> float:
    ...
    return total_attacks * avg_dmg / duration
...
def _phase_weighted_dps(
    ...
    for r in rotations:
        ...
        if w <= 0:
            continue
        weighted_sum += w * _rotation_attack_dps(stats, r, target_armor, mode_dmg_mult)
        total_weight += w
    if total_weight <= 0:
        return 0.0
...
def compute_dps(
    ...
    phase_dps = {
        p: _phase_weighted_dps(stats, rotations_by_phase[p], target_armor, mode_mult)
        for p in PHASES
    }
    ...
    ad = float(stats.get("ad", 0.0))
    eff_as = float(stats.get("as", 0.0))
    avg_attack_dmg = ad * (1 + crit * DEFAULT_CRIT_BONUS) * _armor_factor(target_armor) * mode_mult
    raw_attack_dps = ad * eff_as * (1 + crit * DEFAULT_CRIT_BONUS)

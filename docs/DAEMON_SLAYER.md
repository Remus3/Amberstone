...
**Status: FUNCTIONALLY COMPLETE** - ENGINE_VERSION 1.282.0 - patch 16.18.1.
...
- Item coverage at patch 16.18.1, every figure stated beside the population it counts (all re-derived from the data by `tests/test_docs_ds_item_coverage_drift.py`, so a patch bump that moves them fails loudly): `ITEM_EFFECTS` registry 549 entries; DDragon ships 706 items total, of which 544 carry the bare `gold.purchasable` flag and 468 clear the engine's own shop gate `_is_purchasable` (`gold.purchasable` AND `gold.total` > 0 - `agents/daemon_slayer/rank.py:658`, the predicate `_filter_candidates` actually applies, so 468 is the population the scorers can ever rank). 467 of those 468 carry an `ITEM_EFFECTS` entry; the lone uncredited row is `550007` Party Favor, a `55xxxx` cosmetic that the batch-55 coverage gate excludes by namespace, so coverage over that gate's own population is 466/466. The former `547/547` was ONE number written twice: 547 was `len(ITEM_EFFECTS)` on 2026-05-06 and has never matched any purchasable count at any patch on disk (RM-203).
- patch 16.18.1
...
| `_effects_data.py` | **s246** - the `ItemEffect` registry: 549 entries (measured `len(ITEM_EFFECTS)` 2026-08-13), DDragon purchasable coverage COMPLETE. Patch-pinned per `current.txt`; refresh on patch bump |
...
| `tests/` | The engine's own suite. **The count is deliberately NOT written down here or in the line-5 banner** - measure it with `pytest agents/daemon_slayer --collect-only -q` from the repo root. It is unguarded (`tests/test_docs_daemon_slayer_drift.py` pins ENGINE_VERSION, patch and the route list, and its docstring says it deliberately does not pin `def test_` counts), so any recital drifts on the next test added and nothing fails. That is exactly what happened four times - 10594, 10578, 10605, 10653 - the last of them stale by 31 within a day. Deleting the recital is the fix; do NOT restore one, and do NOT add a guard that pins it, which would make every new test a two-file change (RM-210) |

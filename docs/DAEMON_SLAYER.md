...
**Status: FUNCTIONALLY COMPLETE** - ENGINE_VERSION 1.220.0 - 8564 tests - patch 16.14.1.
...
| `hybrid.py` | **Phase 2 (s175)** - `compute_hybrid()` + `HybridResult` + `rank_items_by_hybrid()` - Bruiser hybrid scorer; composes `compute_dps` x `compute_ehp` weighted by per-champion (alpha,beta) from `archetype_weights.json`; normalized-percentage-delta sort keeps weights intuitive across the ~10x DPS/EHP magnitude gap |

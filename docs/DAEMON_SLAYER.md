...
**Status: FUNCTIONALLY COMPLETE** - ENGINE_VERSION 1.278.1 - patch 16.15.1.
...
- Item coverage at patch 16.15.1, every figure stated beside the population it counts (all re-derived from the data by `tests/test_docs_ds_item_coverage_drift.py`, so a patch bump that moves them fails loudly): `ITEM_EFFECTS` registry 549 entries; DDragon ships 706 items total, of which 544 carry the bare `gold.purchasable` flag and 468 clear the engine's own shop gate `_is_purchasable` (`gold.purchasable` AND `gold.total` > 0 - `agents/daemon_slayer/rank.py:652`, the predicate `_filter_candidates` actually applies, so 468 is the population the scorers can ever rank). 467 of those 468 carry an `ITEM_EFFECTS` entry; the lone uncredited row is `550007` Party Favor, a `55xxxx` cosmetic that the batch-55 coverage gate excludes by namespace, so coverage over that gate's own population is 466/466. The former `547/547` was ONE number written twice: 547 was `len(ITEM_EFFECTS)` on 2026-05-06 and has never matched any purchasable count at any patch on disk (RM-203).
- patch 16.15.1

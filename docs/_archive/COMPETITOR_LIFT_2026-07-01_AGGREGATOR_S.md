...
SHIPPED (R54, 2026-07-01, merge `7f7b82cb` / feat `f9d87451`): the collapse orders Q/W/E by the level each reaches its 5th point (tie-break first-appearance); `ult_levels` = the R positions. TDD RED-first 17 route tests; read-only verifier CONFIRM 8/8; 5-phase UI audit 0 MUST-FIX; live-probed on `:8888` (Aatrox Q>E>W, Lux E>Q>W, numeric 266->Aatrox, cache hit). Champ-select PANEL pixel capture OWED (no live champ-select; overlay-only audit surface).
...
| 4. Skill / ability max order display | MED | NOW - SHIPPED | Data already extracted+shipped+loaded (173 champs); surfacing shipped in-run as `/api/ds-skill-order` + champ-select card (merge `7f7b82cb`) |

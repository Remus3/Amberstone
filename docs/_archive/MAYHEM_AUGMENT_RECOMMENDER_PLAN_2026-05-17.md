...
- **Endpoint:** `GET https://data.v2.iesdev.com/api/v1/query_objects/prod/lol/aram_mayhem_augments` (overlay-app-e's data backend). **Unauthenticated**, no params, `access-control-allow-origin: *`, BunnyCDN-cached (~1h). The `overlay-app-e` HTML is bot-protected; this data host is **not** — plain `GET`, no browser/JS-render needed.

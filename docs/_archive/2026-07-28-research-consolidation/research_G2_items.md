...
(`git log -1 -- docs/research/RC2_RESEARCH_pgr.md` -> `9db9cf93` 2026-06-20), so every
...
| # | ITEM | VERDICT | EVIDENCE | EFFORT |
|---|---|---|---|---|
| F1 | `docs io RC peer/` (space in dir name) - KEEP for now, "defer to an operator-gated pass" because renaming hits 10 `.gitignore` secret globs + frozen CLAUDE.md refs | REFUTED | The directory no longer exists: `ls -d "docs io RC peer"` -> **No such file or directory**, and `git ls-files \| grep -i "^docs.?io"` = **empty**. It went with the Peer cross-Claude bridge decommission (`git log -- "docs io RC peer"` -> `6edfbd3e` "OQ2 finish the Peer cross-Claude bridge decommission sweep", then `8c2afe21`). No CLAUDE.md / ARCHITECTURE.md reference survives (grep = empty). The rename question is moot. | - |
...
| S2 | Section 6 follow-up E7 - ARAM bench-swap latency | SHIPPED | `docs/LIVE_GAME_GATED_SYNC.md:1676` E7a (`64591d5f`) tightens the RC-LCUAgent bench-swap queue-drain (fast 0.1s re-poll); `:2047` "Bench-swap state-render responsiveness (E7 TODO-1, commit `fe34040c`) shipped independently, NOT gated". | - |

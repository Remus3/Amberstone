...
| Riot per-role grading rubric calibration | **SHIPPED** | Item 335, `42780b3d`, `core/post_game_rubric.py`. | - |
...
| companion F4 zero-click rune+item import | **SHIPPED** | `lcu/lcu_rune_writer.py` + LEDGER 867; the swap defect was found and fixed same-day (`4b4aeb67`, 13 RED-first tests). File marks it "F4 fully CLOSED". | - |
...
| RM-81 checker `meraki_endpoints` shape bug (concatenated per-level tail read as max-rank) + `SHAPE_SUSPECT` marker | **SHIPPED** | `tools/ds_wiki_staleness_check.py:283` `kept, marker = rank_series(vals, cooldown)` and `:328` `row["SHAPE_SUSPECT"] = suspect[attr]`; guard test `tests/test_ds_wiki_staleness_check.py:470`. LEDGER 947 (`62e4a410`). Note the doc's PRESCRIBED fix (index by `len(cooldown)`) was **REFUTED** - it crashes on Aurelion Sol Q and mis-truncates 12 abilities; the shipped tell is the first INTERNAL DROP. | - |

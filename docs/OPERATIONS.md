...
| `RC-ClaudeQuotaWatch` | Every 2h | Administrator / HIGHEST | Operator utility: `pythonw C:\Users\Administrator\claude_quota_watch.py` reads acct A's weekly `unified7d` from teamclaude and fires ONE Windows toast (per weekly window) at >=90% telling the operator to switch the Claude GUI login from `<operator-email>` to `<operator-email-failover>`. Needed because the MSIX desktop GUI bypasses the proxy, so failover there is a MANUAL account switch. State: `~\.config\claude_quota_watch_state.json` |

...
- **Tier classification (R5)**: Phase-1 districts/presence-module/ward-cap/fusion/macro are Tier-1 local-logic where they add NEW files + own tests, but any edit that changes `/api/state` schema (presence `zoi.districts`, fusion `zoi.districts_fused`, macro callouts path) is Tier-2 -> full DUAL suite (DS dir + `tests/`). No `ENGINE_VERSION`/scorer/item-effect touch anywhere in this program, so NO DS `:8893` restart or Share mirror is required (confirm before claiming - if no engine file changed, skip the DS bounce).
...
- **Check 4 OBS round-trip - VALIDATED (transport); flip pending.** OBS-WS enabled live (auth, port 4455); `_identify` + `grab_source_screenshot("Game Capture")` round-trips a 412KB frame. `config/coach_settings.json` is gitignored (safe to persist the OBS password). NOT yet a blind flip: (a) the grab was intermittent across repeated probes, (b) the OBS Game-Capture frame geometry differs from the desktop grab, so `minimap_rect` needs re-registration for the OBS frame before `obs.frame_source` flips ON. Flip candidate, geometry-gated.

Gate status after this session: `obs.frame_source` OFF (geometry-gated flip candidate), `RC_ZOI_IDENTITY` OFF (blocked on CV precision), roster-wiring OFF (no visible signal at defaults). None flipped - do-not-flip-blind held.

...
| 11 | "The only `slots.hold` hits are inside `tests/test_loop_concurrency.py`" | RSC 1610 section 2 | **ENUMERATION FALSE, substantive claim UNCHANGED.** RSC 1635 section 4 self-corrects: RSC's own `core/config.py`, line 97, also hits, as a COMMENT not a call. That path is in RSC'S TREE and RC has no such file, so it is deliberately written WITHOUT a resolvable `path:line` form - the citation guard resolves every backticked `path:line` against RC's OWN tracked files, and a foreign path in that shape reads to it as a broken citation. RSC 1710 section 5 re-enumerates all seven hits. Nothing outside RSC's tests acquires a slot. |

...
`agents/daemon_slayer/` - 547 item effects, ENGINE_VERSION 1.75.0, 5663 tests, patch 16.11.1. All 4 coach modes DS-before-Haiku. Item 232 (ENGINE 1.68.0) wires 3 owed sidecar buckets ALL OPT-IN/byte-identical: geometry (`geometry.py` AoE-multiplier into burst `aoe_targets_hit`), recharge (`recharge_ledger.py` time-step charge-availability into fight_report 6th section), mode_modifiers (dps/ehp dmg mults urf/ofa/usb/nb + ar/swift growth-addends via `engine._scale_champion_base`, `apply_mode_modifiers` flag) + rune proc-signature lift (`RuneProc.condition` + caster_hp_pct/game_time_s/role/bonus_as; ships Last Stand 8299/Absolute Focus 8233/Gathering Storm 8236 byte-identical default; registry 16->19). Item 233 (ENGINE 1.69.0) wires the missile travel-time bucket (`missile.py` spell_travel_time, distance/missile_speed with speed-band + cone_distance-sentinel gating, into fight_report 7th section) + rune target_hp metadata tags (Cut Down 8017/Coup de Grace 8014, keystone_amp stays unconditional - burst-window approximation) + burst role-from-attackrange (Lethal Tempo ranged 6-24); static-CD accessor data-staged (no consumer); 5 of 6 buckets wired, static-CD accessor-staged, cc_tags operator-CLOSED; flips (gate_ammo/apply_mode_modifiers/aoe_targets_hit) stay default-OFF operator-gated. Item 234 (ENGINE 1.70.0) mana_sim opt-in `apply_ability_haste` CDR model (reduces cooldown-bearing slots via `total_item_ability_haste` + Riot `base_cd/(1+AH/100)`, byte-identical off / no-AH, binds on cooldown-repeating sequences) + static-CD HONEST no-consumer verdict (QWER coverage 3 abilities with a mislabel, too unreliable to gate haste-immunity); all 6 sidecar buckets RESOLVED. Item 235 (ENGINE 1.71.0) self-audit wiring + parameterize pass threads all 6 opt-in scoring flags end-to-end into the rank_* wrappers + :8893 routes (apply_mode_modifiers -> rank/dps/ehp/rank-tank/hybrid/rank-bruiser/beam/dps_curve/fight_report; aoe_targets_hit -> rank-assassin/burst; gate_ammo + apply_ability_haste -> fight-report -> mana_sim) + wires the previously routeless `enemy_champions` (NEW route arg `enemies`) + `include_conditional` into /ehp /hybrid /rank-bruiser so the cc_blended_ehp discount is route-reachable - ALL OPT-IN/byte-identical at default; DRY shared `_item_ability_haste.effective_cooldown` haste-CDR helper; NEW behavior-neutral GET /modifier-summary diagnostic surfaces `modifier_blocks.summarize_modifiers`. Item 236 (ENGINE 1.72.0) opt-in CC-adjusted EHP ranking: `rank_items_by_ehp(score_by="cc_blended")` + `enemy_champions`/`include_conditional`/`apply_build_tenacity` ranks tanks on enemy-CC-lockdown-adjusted EHP, made build-DEPENDENT by NEW `_item_tenacity.py` (17-item description-parsed tenacity registry + multiplicative `total_item_tenacity`) consumed by `compute_ehp(apply_build_tenacity=...)` scaling caster-eaten enemy CC by (1-tenacity) via the `effective_cc_duration` seam (tenacity items rise vs a non-saturating CC comp); EhpRankedItem +cc_blended_ehp; DEFAULT score_by="blended" byte-identical. Item 237 (ENGINE 1.74.0) symmetric completion - the BRUISER scorer (compute_hybrid + rank_items_by_hybrid + /hybrid + /rank-bruiser) is now tenacity-aware too via the same apply_build_tenacity + score_by="cc_blended" mechanism (the hybrid ranker sorted on PRE-cc blended_ehp; now its delta_pct uses cc_blended when score_by="cc_blended"); both tank + bruiser cc_blended scorers tenacity-aware, DEFAULT byte-identical. **Six archetype scorers** dispatch via `rank_for_primary_archetype()` in `core/daemon_slayer_client.py` (no fallbacks): carry → `ds.dps`, tank → `ds.ehp`, bruiser → `ds.hybrid` (α·dps + β·ehp; per-champion weights in `archetype_weights.json`), mage → `ds.ability` (per-spell DPS via `ability_dps.py` + measured cast rates from `spell_cast_rates.json`), assassin → `ds.burst` (combo-window evaluator in `burst.py`), enchanter → `ds.hps` (curated formulas in `data/daemon_slayer/<patch>/enchanter_items.json`). cc_conditional ecosystem COMPLETE with 5 consumer surfaces: cc_pressure (engine 1.38.0) + compute_ehp (1.39.0) + compute_hybrid (1.39.0) + `core/cc_conditional_impact_context.py` coach prompt (1.40.0) + `dashboard/routes_cc_conditional_pressure.py` dashboard UI (1.40.0); 72 entries (63 primary + 8 sidecar) / 57 champs across waves 0-23 (wave 7 ENGINE 1.44.0 schema-lift forward-marker added 12 condition tags incl. COND_FRENZY_STATE + COND_RANGE_GATED; wave 17 ENGINE 1.54.0 added 13th tag COND_TRAVERSE; wave 18 ENGINE 1.55.0 adds `coexists_with_unconditional` flag + MAX-rule semantics + Maokai R COND_RANGE_GATED first consumer + Briar R COND_TARGET_DEBUFFED (SECOND 3-slot champ after TahmKench); wave 19 ENGINE 1.56.0 Meraki extractor state-tracking schema lift adds `parent_resource` field per spell form (FURY / BLOOD_WELL / FRENZY / RAGE / HEAT / ENERGY / GRIT / MANA) - 0 net-new entries; ALL 6 brief-named wave-7 schema-lift carries verified ALREADY-SHIPPED via prior waves + 3 REJECT-verified - forward-marker infrastructure for future COND_FRENZY_STATE consumers; wave 8 ENGINE 1.45.0 added Morgana R + Seraphine E + Evelynn W; wave 9 ENGINE 1.46.0 Meraki extractor schema lift adds `effects_descriptions` field + closes Renekton W frenzy_state + Hwei E form 1 + Neeko E; wave 10 ENGINE 1.47.0 same-spell-slot sidecar registry schema lift closes Karma W form_index=1 R-empowered Renewal root + Hwei E form_index=2 Gaze of the Abyss root; wave 11 ENGINE 1.48.0 (item 156) closes Sion R Unstoppable Onslaught primary channel_completion + Gnar W form_index=1 Wallop sidecar Mega-form-gated frenzy_state 3rd consumer; wave 12 ENGINE 1.49.0 (item 170) closes Singed E target_debuffed root + Alistar E nth_hit stun + Sylas E form_index=1 sidecar channel_completion stun; wave 13 ENGINE 1.50.0 (item 171) closes Sejuani E nth_hit + Renata Q + Shaco R + Warwick E channel_completion + Fizz R + Aphelios Q form 3 sidecar target_debuffed + TahmKench W channel_completion making TahmKench FIRST 3-slot cc_conditional champ; wave 14 ENGINE 1.51.0 (item 172) Meraki extractor cast_time schema lift + Jayce E form 0 cast-time root sidecar channel_completion FIRST Jayce CC registration anywhere; wave 15 ENGINE 1.52.0 (item 173) closes Zac Q nth_hit different-target root + Ornn E terrain-collision stun FIRST COND_TERRAIN consumer + Rell W form 0 sidecar channel_completion stun; wave 16 ENGINE 1.53.0 (item 174) closes Garen Q nth_hit empowered-AA silence FIRST Garen CC + Syndra E target_debuffed Dark-Sphere knockback stun FIRST Syndra CC + Udyr E nth_hit empowered-AA stun FIRST Udyr CC + KSante W channel_completion recast stun multi-wave coexistence with wave 1 KSante Q; wave 17 ENGINE 1.54.0 (item 175) adds COND_TRAVERSE tag + Taliyah E Unweaver's Wall traversal knockup nth_hit 0.75s multi-wave coexistence with Taliyah W wave 1; wave 20 ENGINE 1.58.0 (item 188 `7530002`) Meraki extractor `notes` field schema lift per spell form + Vayne E Condemn terrain-collision stun 1.5s primary COND_TERRAIN coexists_with_unconditional=True SECOND COND_TERRAIN consumer after Ornn E wave 15 FIRST Vayne first-order CC coexists with 0.5s knockback in _PER_SPELL_CC_DURATIONS; REJECTS Kindred E + Diana P + Vayne P; wave 21 ENGINE 1.59.0 (item 189 `4d60420`) cc_conditional re-audit against `notes` field schema lift + Xin Zhao Q 3rd-attack knockup nth_hit 0.75s + Xin Zhao R target-NOT-Challenged stun target_debuffed 0.75s INVERSE-gated FIRST Xin Zhao Q + R first-order CC anywhere - Xin Zhao becomes 2-slot champion in same wave; 38 REJECTS). CS archetype-picker UI lives in the champ-select view (My Pick card 6-button grid) + `core/archetype_picks.py` storage + `dashboard/routes_archetype.py`. Coach integration via `coach_integration/archetype_dispatch.py` resolves the archetype via `core.archetype_picks.get_archetype_for` + dispatches to the right scorer; all 4 mode coaches consume the helper. State-builder stamps `state.cs_archetype_pick` for the active champion (decorative for picker UI). First-purchase mismatch nudge live (`/api/archetype-nudge` + chip renderer in dashboard). Per-(champion, key) override registries: `champion_max_priority.json`, `champion_combo_sequences.json`, `champion_form_index.json`, `champion_block_index.json` (196 entries across 125 champions, 73% roster coverage). Dead-unique candidates (Trinity→ER etc.) filtered by default. See `docs/DAEMON_SLAYER.md`.

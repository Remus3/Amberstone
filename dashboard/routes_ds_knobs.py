...
SCOPED v1 - exposes ONLY the three knobs that map to existing engine args:
...
The lolsolved fight-length-reweight knob is DEFERRED (it is the only one
needing a new engine arg) and is out of scope for v1.

Request shape:
  GET /api/ds-knobs?champion=<champ>[&mode=SR][&items=<id,id>]
      [&target_armor=<f>][&target_mr=<f>][&budget=<int>][&level=<1..18>]
...
target_armor, target_mr, budget, level). Mirrors routes_cooldown_watch
cache discipline.
...
Don't-redo:
  ...
  * The fight-length-reweight knob is DEFERRED. It needs a new engine arg
    (a weight multiplier over compute_dps's phase rotation); do NOT add it
    here without an ENGINE bump + the framed scope question in the spec.
...
def _cache_key(champion, mode, items, armor, mr, budget, level) -> tuple:
    return (
        champion,
        mode,
        tuple(sorted(items)),
...
def _serve_ds_knobs(h) -> None:
    ...
    try:
        ...
        key = _cache_key(champion, mode, items, armor_override, mr_override,
                         budget, level)
        now = time.time()
        with _CACHE_LOCK:
